...
function App() {
  ...
  return (
    <div className="relative min-h-screen w-full bg-gray-950 text-white font-sans">
      ...
      <main className="flex flex-col items-center w-full px-4 text-center py-12 sm:py-4">
        ...
        <div className="">
            ...
            <p className="text-sm cursor-default mb-4  font-bold  sm:text-base text-gray-500 max-w-xl mx-auto">
        ...
        {isLoading && <p className="mt-2 text-lg text-blue-400">Đang phân tích ảnh, vui lòng chờ...</p>}
        {error && <p className="mt-8 text-lg text-red-500">{error}</p>}

        {isSongListVisible && suggestionData && (
          <div ref={songListRef} className="w-full mt-12 mb-10 sm:mt-16">
            <SongList 
      ...
      <footer className="absolute cursor-default pt-10 font-bold bottom-0 left-0 right-0 text-center p-5 text-gray-700 text-xs sm:text-sm">
      </footer>
    </div>
  );
}
